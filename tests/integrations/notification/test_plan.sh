...
DOCKER_OUTPUT_DIR="/tests/output/notification"
...
docker run --rm \
  ...
  jmeter-base:latest jmeter \
    ...
    -j $DOCKER_OUTPUT_DIR/jmeter-output/jmeter-notification.log \
    -l $DOCKER_OUTPUT_DIR/jmeter-output/jmeter-testing-notification.jtl \
